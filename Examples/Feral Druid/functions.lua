...
function functions.isInPandemicRemainingTime(target,auraID)
    local valid = true
    if auraID ~= nil then
        if game_api.unitHasAura(target,auraID,true) then
            local globalTime = game_api.unitAuraEndTime(target,auraID,true) - game_api.unitAuraStartTime(target,auraID,true)
...
function functions.replaceSpell(target,auraID)
    local valid = true
    if auraID ~= nil then
        if game_api.unitHasAura(target,auraID,true) then
            local globalTime = game_api.unitAuraEndTime(target,auraID,true) - game_api.unitAuraStartTime(target,auraID,true)
...
function functions.CanCast(spell)
    ...
    return game_api.canCast(spell) and game_api.spellIsKnown(spell) and not game_api.isOnCooldown(spell)  and game_api.getCooldownRemainingTime(spell) == 0 
...
function functions.PartyUnitMissingAura(auraID)
    ...
    for _, unit in ipairs(game_api.getPartyUnits()) do
        if game_api.unitHealthPercent(unit) > 0 and functions.IsInRange(30, unit) then
